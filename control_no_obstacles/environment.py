...
class Env(object):
    ...
    def _reward(self):
        ...
        while True:
            ...
            if dis > 6:
                ...
                errorYaw /= 2
                errorYaw = self.track()[(index+3) % len(self.track())].yaw() - self.pose().orientation.x
                ref_yaw = self.compute_ref_yaw(index) + math.pi/2
                #print(ref_yaw)
                #print(self.pose().orientation.x)
                errorYaw = ref_yaw - self.pose().orientation.x
                #print(errorYaw)
                ...
                reward = math.cos(min(max(errorYaw * 2.3, -math.pi), math.pi) )
                #reward = reward/5
    ...
    #def render(self, terminal, pose, speed):
        ...
        reward = round(velocity / 8,3) * self._reward() / 3
        reward = round(reward,3)
        if terminal:
            reward = -3
            reward = -5
        return image_data, reward, terminal, self._intersection()
