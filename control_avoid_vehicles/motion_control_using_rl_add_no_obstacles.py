...
import environment_no_vehicles as En
...
OBSERVE = 16000. # timesteps to observe before training
...
EXPLORE = 30000000. # frames over which to anneal epsilon
...
def enjoyPrius(args):
    ...
    while going_on:
        ...
        if step % 10000 == 0:
            saver.save(sess, 'zero_networks_add/' + GAME + '-dqn', global_step = step)
        ...
        base_angle_t = math.sin(max(-1*math.pi * 0.5, min(math.pi * 0.5,errorYaw *2.5))) * 7
        base_angle_t = round(base_angle_t,2)
        if step % 5 ==0 or terminal:
            printPink("   action from q value : "+str(action_angle_t) )
        ...
        if random.random() < epsilon:
            action_array_t, action_angle_t = getRandomAction(qV_t)
            action_angle_t = round(action_angle_t,2)
            if step % 5 ==0 or terminal:
                printYellow("   action from random : "+str(action_angle_t) )
                print("base angle is : " + str(base_angle_t))
                print("sum angle is : " + str(base_angle_t + action_angle_t))

        prius.control_prius(0.6, action_angle_t , 0)
        #prius.control_prius(0.6, base_angle_t + action_angle_t , 0)
...
def getAction(qv):
    ...
    action_angle = (np.argmax(qv) - 7) * 1.05
...
def getRandomAction(qv):
    ...
    action_angle = random_value * 1.05
